...
app.post("/files", (req, res) => {
  ...
  if (req.files.upload instanceof Array) {
    for (var i = 0; i < req.files.upload.length; i++) {
      ...
      caches[file]
        .then(() =>
        `Congrats! Your file has been successfully uploaded - you can download your file right here: <button><a type="button" target="_blank" href="/uploaded/${file}">Download</a></button>`
        )
        .catch((error) => {
          console.log(error);
          res.end(error);
